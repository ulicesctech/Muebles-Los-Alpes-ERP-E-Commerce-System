CREATE OR REPLACE PACKAGE BODY PKG_ADMIN_LOGIN_EMPLEADO AS
    ...
    -- ══════════════════════════════════════
    -- PRIVADO: Validar que el empleado exista
    -- ══════════════════════════════════════
    ...
    BEGIN
        ...
        IF v_count = 0 THEN
            RAISE_APPLICATION_ERROR(-20007,
                'Empleado ID ' || p_em_empleado || ' no existe en RH_EMPLEADO');
        END IF;
    END assert_empleado;
    ...
    BEGIN
        ...
        INSERT INTO ADMIN_LOGIN_EMPLEADO (
            em_empleado, logem_usuario, logem_password
        ) VALUES (
            p_em_empleado,
            TRIM(p_usuario),
            p_password  -- <-- sin hash por ahora
        );

        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Login empleado creado: ' || p_em_empleado);
    EXCEPTION
    ...
    BEGIN
        ...
        SET logem_password = p_password  -- <-- sin hash
        WHERE em_empleado = p_em_empleado;
        ...
        IF v_rows = 0 THEN
            RAISE_APPLICATION_ERROR(-20004,
                'Login no encontrado para empleado: ' || p_em_empleado);
        END IF;

        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Password actualizado: ' || p_em_empleado);
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE;
    ...
    BEGIN
        ...
        UPDATE ADMIN_LOGIN_EMPLEADO
        SET logem_usuario = TRIM(p_usuario)
        WHERE em_empleado = p_em_empleado;
        ...
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Usuario actualizado: ' || p_em_empleado);
    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20006, 'Usuario ya existe');
    ...
    BEGIN
        ...
        DELETE FROM ADMIN_LOGIN_EMPLEADO
        WHERE em_empleado = p_em_empleado;
        ...
        COMMIT;
        DBMS_OUTPUT.PUT_LINE('Login eliminado: ' || p_em_empleado);
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE;
    ...
    BEGIN
        BEGIN
            ...
            AND   logem_password = p_password; -- <-- sin hash
