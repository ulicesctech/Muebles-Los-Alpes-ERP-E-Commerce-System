CREATE OR REPLACE PACKAGE BODY PKG_BOD_TIPO AS
  ...
  BEGIN
    IF TRIM(p_descripcion) IS NULL THEN
      RAISE_APPLICATION_ERROR(-21201, 'BOD_TIPO: descripcion obligatoria.');
    END IF;
    IF p_cat_categoria IS NULL THEN
      RAISE_APPLICATION_ERROR(-21202, 'BOD_TIPO: cat_categoria obligatoria.');
    END IF;
  ...
  BEGIN
    IF p_id IS NULL THEN
      RAISE_APPLICATION_ERROR(-21203, 'BOD_TIPO: id obligatorio.');
    END IF;
    IF TRIM(p_descripcion) IS NULL THEN
      RAISE_APPLICATION_ERROR(-21204, 'BOD_TIPO: descripcion obligatoria.');
    END IF;
    IF p_cat_categoria IS NULL THEN
      RAISE_APPLICATION_ERROR(-21205, 'BOD_TIPO: cat_categoria obligatoria.');
    END IF;
    ...
    IF SQL%ROWCOUNT = 0 THEN
      RAISE_APPLICATION_ERROR(-21206, 'BOD_TIPO: no existe.');
    END IF;
  END;
  ...
  BEGIN
    IF p_id IS NULL THEN
      RAISE_APPLICATION_ERROR(-21207, 'BOD_TIPO: id obligatorio.');
    END IF;
    ...
    IF v_used > 0 THEN
      RAISE_APPLICATION_ERROR(-21208, 'BOD_TIPO: no se puede eliminar, está usado por productos.');
    END IF;

    DELETE FROM BOD_TIPO WHERE tip_tipo = p_id;

    IF SQL%ROWCOUNT = 0 THEN
      RAISE_APPLICATION_ERROR(-21209, 'BOD_TIPO: no existe.');
    END IF;
  END;
  ...
  BEGIN
    IF p_cat_categoria IS NULL THEN
      RAISE_APPLICATION_ERROR(-21210, 'BOD_TIPO: cat_categoria obligatoria.');
    END IF;
