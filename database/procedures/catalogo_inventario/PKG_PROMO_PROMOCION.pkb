CREATE OR REPLACE PACKAGE BODY PKG_PROMO_PROMOCION AS
  ...
  BEGIN
    ...
    IF v_ref IS NULL THEN
      RAISE_APPLICATION_ERROR(-21901, 'PROMO_PROMOCION: referencia obligatoria.');
    END IF;

    IF p_porcentaje IS NULL OR p_porcentaje <= 0 OR p_porcentaje > 100 THEN
      RAISE_APPLICATION_ERROR(-21902, 'PROMO_PROMOCION: porcentaje debe ser >0 y <=100.');
    END IF;

    IF p_fecha_inicio IS NULL OR p_fecha_final IS NULL THEN
      RAISE_APPLICATION_ERROR(-21903, 'PROMO_PROMOCION: fechas obligatorias.');
    END IF;

    IF p_fecha_inicio > p_fecha_final THEN
      RAISE_APPLICATION_ERROR(-21904, 'PROMO_PROMOCION: fecha_inicio no puede ser mayor a fecha_final.');
    END IF;
  ...
  BEGIN
    IF p_id IS NULL THEN
      RAISE_APPLICATION_ERROR(-21905, 'PROMO_PROMOCION: id obligatorio.');
    END IF;

    DELETE FROM PROMO_PROMOCION WHERE prom_promocion = p_id;

    IF SQL%ROWCOUNT = 0 THEN
      RAISE_APPLICATION_ERROR(-21906, 'PROMO_PROMOCION: no existe.');
    END IF;
  END;
  ...
  BEGIN
    v_ref := TRIM(p_pro_referencia);
    IF v_ref IS NULL THEN
      RAISE_APPLICATION_ERROR(-21907, 'PROMO_PROMOCION: referencia obligatoria.');
    END IF;
  ...
  BEGIN
    v_ref := TRIM(p_pro_referencia);
    IF v_ref IS NULL THEN
      RAISE_APPLICATION_ERROR(-21908, 'PROMO_PROMOCION: referencia obligatoria.');
    END IF;
