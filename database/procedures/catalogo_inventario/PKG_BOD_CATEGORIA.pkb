CREATE OR REPLACE PACKAGE BODY PKG_BOD_CATEGORIA AS
  ...
  BEGIN
    IF TRIM(p_descripcion) IS NULL THEN
      RAISE_APPLICATION_ERROR(-21001, 'BOD_CATEGORIA: descripcion obligatoria.');
    END IF;
  ...
  BEGIN
    IF p_id IS NULL THEN
      RAISE_APPLICATION_ERROR(-21002, 'BOD_CATEGORIA: id obligatorio.');
    END IF;

    IF TRIM(p_descripcion) IS NULL THEN
      RAISE_APPLICATION_ERROR(-21003, 'BOD_CATEGORIA: descripcion obligatoria.');
    END IF;
    ...
    IF SQL%ROWCOUNT = 0 THEN
      RAISE_APPLICATION_ERROR(-21004, 'BOD_CATEGORIA: no existe.');
    END IF;
  END;
  ...
  BEGIN
    IF p_id IS NULL THEN
      RAISE_APPLICATION_ERROR(-21005, 'BOD_CATEGORIA: id obligatorio.');
    END IF;
    ...
    IF v_used > 0 THEN
      RAISE_APPLICATION_ERROR(-21006, 'BOD_CATEGORIA: no se puede eliminar, tiene tipos asociados.');
    END IF;

    DELETE FROM BOD_CATEGORIA WHERE cat_categoria = p_id;

    IF SQL%ROWCOUNT = 0 THEN
      RAISE_APPLICATION_ERROR(-21007, 'BOD_CATEGORIA: no existe.');
    END IF;
  END;
