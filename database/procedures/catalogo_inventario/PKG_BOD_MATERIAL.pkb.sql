CREATE OR REPLACE PACKAGE BODY PKG_BOD_MATERIAL AS
  ...
  BEGIN
    IF TRIM(p_descripcion) IS NULL THEN
      RAISE_APPLICATION_ERROR(-20001, 'BOD_MATERIAL: descripcion obligatoria.');
    END IF;
  ...
  BEGIN
    IF p_id IS NULL THEN
      RAISE_APPLICATION_ERROR(-20002, 'BOD_MATERIAL: id obligatorio.');
    END IF;
    IF TRIM(p_descripcion) IS NULL THEN
      RAISE_APPLICATION_ERROR(-20003, 'BOD_MATERIAL: descripcion obligatoria.');
    END IF;
    UPDATE BOD_MATERIAL
       SET mat_descripcion = TRIM(p_descripcion)
     WHERE mat_material = p_id;
    IF SQL%ROWCOUNT = 0 THEN
      RAISE_APPLICATION_ERROR(-20004, 'BOD_MATERIAL: no existe.');
    END IF;
  END;
  ...
  BEGIN
    IF p_id IS NULL THEN
      RAISE_APPLICATION_ERROR(-20005, 'BOD_MATERIAL: id obligatorio.');
    END IF;
    SELECT COUNT(1) INTO v_used FROM BOD_PRODUCTO WHERE mat_material = p_id;
    IF v_used > 0 THEN
      RAISE_APPLICATION_ERROR(-20006, 'BOD_MATERIAL: no se puede eliminar, esta usado por productos.');
    END IF;
    DELETE FROM BOD_MATERIAL WHERE mat_material = p_id;
    IF SQL%ROWCOUNT = 0 THEN
      RAISE_APPLICATION_ERROR(-20007, 'BOD_MATERIAL: no existe.');
    END IF;
  END;
