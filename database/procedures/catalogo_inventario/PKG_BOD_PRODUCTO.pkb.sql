CREATE OR REPLACE PACKAGE BODY PKG_BOD_PRODUCTO AS
  ...
  BEGIN
    ...
    IF p_alto_cm <= 0 OR p_ancho_cm <= 0 OR p_profundidad_cm <= 0 THEN
      RAISE_APPLICATION_ERROR(-20005, 'BOD_PRODUCTO: dimensiones deben ser > 0.');
    END IF;
    IF p_peso IS NULL OR p_peso <= 0 THEN
      RAISE_APPLICATION_ERROR(-20006, 'BOD_PRODUCTO: peso debe ser > 0.');
    END IF;
  ...
  BEGIN
    ...
    IF p_alto_cm <= 0 OR p_ancho_cm <= 0 OR p_profundidad_cm <= 0 THEN
      RAISE_APPLICATION_ERROR(-20011, 'BOD_PRODUCTO: dimensiones deben ser > 0.');
    END IF;
    IF p_peso IS NULL OR p_peso <= 0 THEN
      RAISE_APPLICATION_ERROR(-20012, 'BOD_PRODUCTO: peso debe ser > 0.');
    END IF;
    UPDATE BOD_PRODUCTO
