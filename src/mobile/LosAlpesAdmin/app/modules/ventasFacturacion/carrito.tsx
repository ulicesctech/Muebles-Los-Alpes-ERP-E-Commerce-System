...
export default function CarritoScreen() {
  ...
  const [correlativosFacturados, setCorrelativosFacturados] = useState<
    Set<string>
  >(new Set());
  ...
  const [hvPrecioSeleccionado, setHvPrecioSeleccionado] = useState<
    number | null
  >(null);
  ...
  const cargarDatos = async () => {
    try {
      ...
      console.log(
        "[Carrito] Correlativos facturados:",
        JSON.stringify(correlativos),
      );
      console.log(
        "[Carrito] PRE_CORRELATIVO de carritos:",
        JSON.stringify(listaCarritos.map((c) => c.PRE_CORRELATIVO)),
      );
  ...
  const cargarClientes = async () => {
    try {
      ...
      console.log("[cargarClientes] result:", JSON.stringify(result));
      const lista = Array.isArray(result) ? result : (result?.data ?? []);
      setClientes(lista);
      if (lista.length > 0) setClienteId(lista[0].cli_cliente);
    } catch (error: any) {
      console.log("[cargarClientes] error:", error?.message);
  ...
  const abrirModalAgregar = (item: Carrito) => {
    ...
    setCantidad("1");
    setHvPrecioSeleccionado(
      productos.length > 0 ? productos[0].HV_HISTORIAL_PRECIO_VENTA : null,
    );
  ...
  const handleAgregarDetalle = async () => {
    ...
    try {
      ...
      await agregarDetalle(
        carritoActivo.PRE_CARRITO,
        hvPrecioSeleccionado,
        cant,
      );
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const estaFacturado =
            item.PRE_CORRELATIVO != null &&
            correlativosFacturados.has(item.PRE_CORRELATIVO);
          const sinProductos =
            !item.PRODUCTOS || item.PRODUCTOS === "Sin productos";
          console.log(
            `[Carrito] carrito ${item.PRE_CORRELATIVO} | Set: ${JSON.stringify([...correlativosFacturados])} | estaFacturado: ${estaFacturado}`,
          );
          return (
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <Text style={styles.total}>
                  Q {parseFloat(String(item.PRE_TOTAL || "0")).toFixed(2)}
                </Text>
              </View>
              ...
              <View style={styles.acciones}>
                <TouchableOpacity
                  style={[
                    styles.btnAgregar,
                    estaFacturado && styles.btnDisabled,
                  ]}
                  onPress={() => abrirModalAgregar(item)}
              ...
              <View style={styles.acciones}>
                {estaFacturado ? (
                  <TouchableOpacity
                    style={[styles.btnFactura, styles.btnFacturado]}
                    disabled
                  >
                  ...
                  <TouchableOpacity
                    style={[
                      styles.btnFactura,
                      sinProductos && styles.btnDisabled,
                    ]}
                  ...
                  >
                    <Text style={styles.btnTexto}>
                      {sinProductos ? "Sin productos" : "Facturar"}
                    </Text>
                  </TouchableOpacity>
                )}
              </View>
              ...
              <ScrollView style={styles.listaWrapper} nestedScrollEnabled>
                {clientes.map((c) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.opcionItem,
                      clienteId === c.cli_cliente && styles.opcionItemSelected,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.opcionText,
                        clienteId === c.cli_cliente &&
                          styles.opcionTextSelected,
                      ]}
                    >
          ...
          <View style={styles.modalBox}>
            ...
            <ScrollView style={styles.listaWrapper} nestedScrollEnabled>
              {productos.map((p) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.opcionItem,
                    hvPrecioSeleccionado === p.HV_HISTORIAL_PRECIO_VENTA &&
                      styles.opcionItemSelected,
                  ]}
                  onPress={() =>
                    setHvPrecioSeleccionado(p.HV_HISTORIAL_PRECIO_VENTA)
                  }
                >
                  <Text
                    style={[
            ...
            <View style={styles.modalAcciones}>
              ...
              <TouchableOpacity
                style={[
                  styles.btnCrear,
                  guardandoDetalle && styles.btnDisabled,
                ]}
...
const styles = StyleSheet.create({
  ...
  modalOverlay: {
    flex: 1,
    backgroundColor: "rgba(47,27,15,0.6)",
    justifyContent: "center",
    alignItems: "center",
  },
  modalBox: {
