...
import { buscarProductos, listarCategorias, listarProductos } from '../../../services/cliente/catalogoService';
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchBar}>
        ...
        <TouchableOpacity style={styles.carritoBtn} onPress={() => router.push('/modules/cliente/carrito' as any)}>
      ...
      <View style={styles.sidebarCard}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.catScroll}>
          <TouchableOpacity
            style={[styles.catItem, categoriaActiva === 0 && styles.catItemActive]}
            onPress={() => handleCategoria(0)}>
            <Text style={[styles.catText, categoriaActiva === 0 && styles.catTextActive]}>Todas</Text>
          </TouchableOpacity>
          {categorias.map(c => (
            <TouchableOpacity
              key={c.CAT_CATEGORIA}
              style={[styles.catItem, categoriaActiva === c.CAT_CATEGORIA && styles.catItemActive]}
              onPress={() => handleCategoria(c.CAT_CATEGORIA)}>
              <Text style={[styles.catText, categoriaActiva === c.CAT_CATEGORIA && styles.catTextActive]}>
                {c.CAT_DESCRIPCION}
              </Text>
            </TouchableOpacity>
          ))}
              ...
              <View style={styles.prodCard}>
                ...
                <View style={styles.cardImgWrap}>
                  ...
                  {item.PROM_PORCENTAJE && (
                    <View style={styles.badgePromo}>
                      <Text style={styles.badgePromoText}>{item.CAMP_NOMBRE} -{item.PROM_PORCENTAJE}%</Text>
                    </View>
                  )}
                ...
                <View style={styles.cardBody}>
                  ...
                  <View style={styles.cardPrecioWrap}>
                    ...
                    <Text style={[styles.precioFinal, item.PROM_PORCENTAJE && styles.precioPromo]}>
...
const styles = StyleSheet.create({
  ...
  searchBar: { backgroundColor: 'white', padding: 12, flexDirection: 'row', alignItems: 'center', gap: 8, borderBottomWidth: 1, borderBottomColor: '#e8d8c0', flexWrap: 'wrap' },
  searchWrap: { flex: 1, minWidth: 120, position: 'relative', flexDirection: 'row', alignItems: 'center', borderWidth: 2, borderColor: '#e8d8c0', borderRadius: 8, backgroundColor: '#fdf8f3' },
  searchIco: { paddingLeft: 10, fontSize: 14 },
  searchInput: { flex: 1, padding: 9, fontSize: 13, color: '#333' },
  btnGold: { backgroundColor: GOLD, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8 },
  btnGoldText: { color: 'white', fontWeight: 'bold', fontSize: 13 },
  btnOutline: { borderWidth: 2, borderColor: '#e8d8c0', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8 },
  btnOutlineText: { color: CAFE, fontSize: 13 },
  carritoBtn: { position: 'relative', padding: 6 },
  carritoIcon: { fontSize: 24 },
  carritoBadge: { position: 'absolute', top: 0, right: 0, backgroundColor: '#e53e3e', borderRadius: 10, width: 18, height: 18, alignItems: 'center', justifyContent: 'center' },
  carritoBadgeText: { color: 'white', fontSize: 9, fontWeight: 'bold' },
  sidebarCard: { backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#e8d8c0' },
  sidebarHead: { backgroundColor: CAFE, paddingHorizontal: 16, paddingVertical: 10 },
  sidebarHeadText: { color: '#f0d9a0', fontSize: 12, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1 },
  catScroll: { paddingVertical: 8 },
  catItem: { paddingHorizontal: 14, paddingVertical: 8, marginHorizontal: 4, borderLeftWidth: 3, borderLeftColor: 'transparent' },
