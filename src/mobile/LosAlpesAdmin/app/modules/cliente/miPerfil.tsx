...
export default function MiPerfilScreen() {
  ...
  useEffect(() => { if (usuario) cargar(); }, []);

  const cargar = async () => {
    try {
      const res = await obtenerPerfil(usuario!.id);
      setPerfil(res.data);
    } catch (e: any) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TouchableOpacity style={styles.pedidosCard}
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.tipoDocWrap}>
            {['DPI', 'CEDULA', 'PASAPORTE', 'NIT'].map(t => (
              <TouchableOpacity
                key={t}
                style={[styles.tipoDocBtn, perfil.CLI_TIPODOCUMENTO === t && styles.tipoDocBtnActive]}
                onPress={() => update('CLI_TIPODOCUMENTO', t)}>
                <Text style={[styles.tipoDocText, perfil.CLI_TIPODOCUMENTO === t && styles.tipoDocTextActive]}>
                  {t}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <TextInput style={styles.input} value={perfil.CLI_NUMDOCUMENTO}
            onChangeText={v => update('CLI_NUMDOCUMENTO', v)} />
          <Text style={styles.label}>NIT</Text>
          <TextInput style={styles.input} value={perfil.CLI_NIT}
            onChangeText={v => update('CLI_NIT', v)} placeholder="CF" />
        </View>
      </View>
        ...
        <View style={styles.cardBody}>
          {[
            ...
            <View key={campo}>
              ...
              <TextInput style={styles.input} value={perfil[campo] || ''}
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardBody}>
          ...
          <TextInput style={styles.input} value={perfil.CLI_PRIMER_TELEFONO}
            onChangeText={v => update('CLI_PRIMER_TELEFONO', v)} keyboardType="phone-pad" />
          <Text style={styles.label}>TELÉFONO SECUNDARIO</Text>
          <TextInput style={styles.input} value={perfil.CLI_SEGUNDO_TELEFONO || ''}
            onChangeText={v => update('CLI_SEGUNDO_TELEFONO', v)} keyboardType="phone-pad" />
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  hero: { background: 'linear-gradient(135deg,#5C3A1E,#8B5E3C)', backgroundColor: CAFE, padding: 28, margin: 16, borderRadius: 14 } as any,
  heroTitle: { color: '#f0d9a0', fontSize: 24, fontWeight: 'bold', marginBottom: 4 },
  heroSub: { color: '#d4b896', fontSize: 13 },
  pedidosCard: { backgroundColor: 'white', margin: 16, marginTop: 0, borderRadius: 12, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, borderWidth: 1, borderColor: '#e8d8c0', elevation: 2 },
  ...
  card: { backgroundColor: 'white', borderRadius: 12, borderWidth: 1, borderColor: '#e8d8c0', margin: 16, marginTop: 0, overflow: 'hidden', elevation: 2 },
  cardHead: { backgroundColor: CAFE, padding: 14 },
  cardHeadText: { color: '#f0d9a0', fontSize: 14, fontWeight: 'bold' },
  cardBody: { padding: 20 },
  label: { fontSize: 11, fontWeight: 'bold', color: CAFE, textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 5, marginTop: 10 },
  input: { borderWidth: 2, borderColor: '#e8d8c0', borderRadius: 8, padding: 10, fontSize: 14, backgroundColor: '#fdf8f3', color: '#333' },
  tipoDocWrap: { flexDirection: 'row', gap: 8, flexWrap: 'wrap', marginBottom: 8 },
  tipoDocBtn: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, borderWidth: 1.5, borderColor: '#e8d8c0' },
  tipoDocBtnActive: { backgroundColor: GOLD, borderColor: GOLD },
  tipoDocText: { fontSize: 12, fontWeight: 'bold', color: '#888' },
  tipoDocTextActive: { color: 'white' },
  btnGuardar: { backgroundColor: '#276749', margin: 16, padding: 16, borderRadius: 10, alignItems: 'center' },
  btnGuardarText: { color: 'white', fontSize: 15, fontWeight: 'bold' },
  noLogin: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 },
  noLoginIcon: { fontSize: 64, marginBottom: 12 },
  noLoginText: { fontSize: 15, color: '#555', textAlign: 'center', marginBottom: 20 },
  btnLogin: { backgroundColor: CAFE, paddingHorizontal: 28, paddingVertical: 12, borderRadius: 8 },
  btnLoginText: { color: 'white', fontWeight: 'bold', fontSize: 14 },
});