...
<style>
    ...
    .info-banner { background:#fdf6ec; border:1px solid #e8d0a0; border-radius:10px; padding:14px 18px; margin-bottom:20px; font-size:13px; font-family:Arial,sans-serif; color:#7a5818; }
...
<div class="page-title">Gestion de Nichos</div>

<%-- INFO --%>
<div class="info-banner">
    Para crear un nicho nuevo, hacelo desde <a href='<%: ResolveUrl("~/Modules/CatalogoInventario/Almacenes.aspx") %>' style="color:#C9973A; font-weight:bold;">Almacenes</a> — cada nicho se crea y asigna directamente a su almacen.
</div>
...
<%-- FORMULARIO EDICION — solo visible al editar --%>
<asp:Panel ID="pnlFormEditar" runat="server" Visible="false">
    <div class="form-card">
        <div class="form-card-head">
            <span><asp:Label ID="lblTituloForm" runat="server" Text="Editar Nicho"></asp:Label></span>
        </div>
        <div class="form-card-body">
            ...
            <div class="f-row">
                <div class="f-group">
                    <label>Numero *</label>
                    <asp:TextBox ID="txtNumero" runat="server" CssClass="form-control" MaxLength="50" placeholder="A-001"></asp:TextBox>
                </div>
                <div class="f-group">
                    <label>Zona *</label>
                    <asp:TextBox ID="txtZona" runat="server" CssClass="form-control" MaxLength="100" placeholder="Zona Norte"></asp:TextBox>
                </div>
                <div class="f-group">
                    <label>Caracteristica *</label>
                    <asp:TextBox ID="txtCaracteristica" runat="server" CssClass="form-control" MaxLength="300" placeholder="Alta capacidad"></asp:TextBox>
                </div>
            </div>
            <div class="f-row" style="margin-top:12px;">
                <asp:Button ID="btnGuardar"  runat="server" Text="Actualizar" CssClass="btn-gold"    OnClick="btnGuardar_Click" />
...
<div class="table-card">
    <asp:GridView ID="gvNichos" runat="server" AutoGenerateColumns="false" OnRowCommand="gvNichos_RowCommand" GridLines="None">
        <Columns>
            ...
            <asp:BoundField DataField="ALM_NOMBRE"         HeaderText="Almacen"        ItemStyle-Width="180px" />
            <asp:TemplateField HeaderText="Acciones" ItemStyle-Width="160px">
                <ItemTemplate>
                    <div class="actions-cell">
                        <asp:LinkButton CommandName="Editar"   CommandArgument='<%# Eval("NIC_NICHO") %>' runat="server" CssClass="btn-edit-t">Editar</asp:LinkButton>
                        <asp:LinkButton CommandName="Eliminar" CommandArgument='<%# Eval("NIC_NICHO") %>' runat="server" CssClass="btn-del-t" OnClientClick="return confirm('Desea eliminar este nicho?');">Eliminar</asp:LinkButton>
                    </div>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
        <EmptyDataTemplate><div class="empty-state">No hay nichos registrados.</div></EmptyDataTemplate>
    </asp:GridView>
</div>
